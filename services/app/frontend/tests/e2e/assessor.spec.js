...
test('assessor applies a new saving plan proposal with depot selection', async ({ page }) => {
  ...
  await expect(page.getByLabel('Decision for NEW123456789')).toContainText('Block for Saving Plan Proposals')
  await expect(page.getByLabel('Decision for NEW123456789')).toContainText('Block for all Proposals')
...
test('assessor blacklists a proposal without depot selection', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'Run Assessment' }).click()
  await page.getByRole('button', { name: 'Apply Approvals' }).click()
  await expect(page.getByLabel('Decision for NEWBLACKLIST2')).toContainText('Block for Saving Plan Proposals')
  await expect(page.getByLabel('Decision for NEWBLACKLIST2')).toContainText('Block for all Proposals')
