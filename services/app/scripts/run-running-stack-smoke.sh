...
protected_status="$(curl_request -sS -o /dev/null -w "%{http_code}" -H "Authorization: Bearer ${token}" "${PROTECTED_URL}")"
if [ "${protected_status}" != "200" ]; then
  printf 'Protected endpoint check failed for %s with status %s.\n' "${PROTECTED_URL}" "${protected_status}" >&2
  exit 1
fi

logout_status="$(curl_request -sS -o /dev/null -w "%{http_code}" -X POST -H "Authorization: Bearer ${token}" "${AUTH_LOGOUT_URL}")"
if [ "${logout_status}" != "204" ]; then
  printf 'Logout failed with status %s.\n' "${logout_status}" >&2
  exit 1
fi

revoked_status="$(curl_request -sS -o /dev/null -w "%{http_code}" -H "Authorization: Bearer ${token}" "${PROTECTED_URL}")"
