...
public class KnowledgeBaseLlmService implements KnowledgeBaseLlmClient {
    ...
    private static final String FIELD_PE_CURRENT = "pe_current";
    private static final String FIELD_REVENUE = "revenue";
    private static final String FIELD_NET_INCOME = "net_income";
    private static final String FIELD_EBITDA = "ebitda";
    private static final String FIELD_EPS_HISTORY = "eps_history";
    private static final String FIELD_EPS_NORM = "eps_norm";
    private static final String FIELD_PE_LONGTERM = "pe_longterm";
    private static final String FIELD_PE_TTM_HOLDINGS = "pe_ttm_holdings";
    private static final String FIELD_EARNINGS_YIELD_TTM_HOLDINGS = "earnings_yield_ttm_holdings";
    private static final String FIELD_HOLDINGS_COVERAGE_WEIGHT_PCT = "holdings_coverage_weight_pct";
    private static final String FIELD_HOLDINGS_COVERAGE_COUNT = "holdings_coverage_count";
    private static final String FIELD_HOLDINGS_ASOF = "holdings_asof";
    private static final String FIELD_HOLDINGS_WEIGHT_METHOD = "holdings_weight_method";
    private static final String FIELD_PE_METHOD = "pe_method";
    private static final String FIELD_PE_HORIZON = "pe_horizon";
    private static final String FIELD_NET_RENT = "net_rent";
    private static final String FIELD_CITATIONS = "citations";
    private static final String FIELD_PE_CURRENT_ASOF = "pe_current_asof";
    private static final String FIELD_LAYER = "layer";
    private static final String FIELD_LAYER_NOTES = "layer_notes";
    private static final String VALUE_UNKNOWN = "unknown";
    private static final String VALUE_NORMALIZED = "normalized";
    private static final List<String> VALUATION_KEY_ORDER = List.of(
            ...
            FIELD_PE_TTM_HOLDINGS,
            FIELD_EARNINGS_YIELD_TTM_HOLDINGS,
            FIELD_HOLDINGS_COVERAGE_WEIGHT_PCT,
            FIELD_HOLDINGS_COVERAGE_COUNT,
            FIELD_HOLDINGS_ASOF,
            FIELD_HOLDINGS_WEIGHT_METHOD,
            FIELD_PE_METHOD,
            FIELD_PE_HORIZON,
    ...
    public KnowledgeBaseLlmDossierDraft generateDossier(String isin,
        ...
        JsonNode citations = root.get(FIELD_CITATIONS);
    ...
    private String buildNormalizedValuation(String sectionBody, boolean clearHoldings) {
        ...
        if (clearHoldings) {
            values.put(FIELD_PE_TTM_HOLDINGS, VALUE_UNKNOWN);
            values.put(FIELD_EARNINGS_YIELD_TTM_HOLDINGS, VALUE_UNKNOWN);
            values.put(FIELD_HOLDINGS_COVERAGE_WEIGHT_PCT, VALUE_UNKNOWN);
            values.put(FIELD_HOLDINGS_COVERAGE_COUNT, VALUE_UNKNOWN);
            values.put(FIELD_HOLDINGS_ASOF, VALUE_UNKNOWN);
            values.put(FIELD_HOLDINGS_WEIGHT_METHOD, VALUE_UNKNOWN);
        }
        String epsNorm = values.get(FIELD_EPS_NORM);
        String peLong = values.get(FIELD_PE_LONGTERM);
        String peHorizon = values.get(FIELD_PE_HORIZON);
        if (VALUE_NORMALIZED.equals(peHorizon) && (isUnknown(epsNorm) || isUnknown(peLong))) {
            values.put(FIELD_PE_HORIZON, "ttm");
        }
        StringBuilder builder = new StringBuilder();
        for (String key : VALUATION_KEY_ORDER) {
            if (FIELD_EPS_HISTORY.equals(key)) {
                if (!epsLines.isEmpty()) {
                    builder.append("- ").append(FIELD_EPS_HISTORY).append(":\n");
                    for (String epsLine : epsLines) {
                        builder.append(epsLine).append("\n");
                    }
                } else {
                    builder.append("- ").append(FIELD_EPS_HISTORY).append(": ").append(VALUE_UNKNOWN).append("\n");
                }
                continue;
            }
            String value = values.getOrDefault(key, VALUE_UNKNOWN);
    ...
    private String normalizeValueForKey(String key, String value) {
        if (FIELD_HOLDINGS_ASOF.equals(key)) {
            return normalizeHoldingsAsOf(value);
        }
        String normalized = normalizeValue(value);
        if (VALUE_UNKNOWN.equals(normalized)) {
            return normalized;
        }
        ...
        if (normalized.contains("|") && normalized.toLowerCase(Locale.ROOT).contains(VALUE_UNKNOWN)) {
            return VALUE_UNKNOWN;
        }
        return normalized;
    }
    ...
    private String normalizeValue(String value) {
        ...
        if (lower.contains(VALUE_UNKNOWN) || lower.contains("not disclosed") || lower.contains("not provided") || lower.contains("n/a")) {
            return VALUE_UNKNOWN;
        }
    ...
    private boolean isUnknown(String value) {
        return value == null || VALUE_UNKNOWN.equalsIgnoreCase(value.trim());
    }
    ...
    private JsonNode postProcessExtraction(JsonNode payload, String dossierText) {
        ...
        if (valuation != null) {
            applyAsOfFallback(valuation, FIELD_PRICE, "price_asof", researchDate);
            applyAsOfFallback(valuation, FIELD_PE_CURRENT, FIELD_PE_CURRENT_ASOF, researchDate);
            applyAsOfFallback(valuation, "pb_current", "pb_current_asof", researchDate);
            if (VALUE_NORMALIZED.equalsIgnoreCase(textOrNull(valuation, FIELD_PE_HORIZON))
                    && (isNullOrBlank(valuation.get(FIELD_EPS_NORM)) || isNullOrBlank(valuation.get(FIELD_PE_LONGTERM)))) {
                valuation.put(FIELD_PE_HORIZON, "ttm");
            }
            normalizeEpsHistory(valuation, researchDate);
            computePeCurrentIfMissing(valuation, researchDate);
            if (singleStock) {
                if (valuation.get(FIELD_PE_CURRENT) != null && !valuation.get(FIELD_PE_CURRENT).isNull()) {
                    valuation.put(FIELD_PE_METHOD, "ttm");
                    valuation.put(FIELD_PE_HORIZON, "ttm");
                }
                clearSingleStockHoldingsFields(valuation);
            }
            applyPeriodEndFallback(valuation, FIELD_EBITDA, "ebitda_period_end", researchDate);
            applyPeriodEndFallback(valuation, FIELD_NET_RENT, "net_rent_period_end", researchDate);
        ...
        if (financials != null) {
            applyPeriodEndFallback(financials, FIELD_REVENUE, "revenue_period_end", researchDate);
            applyPeriodEndFallback(financials, FIELD_NET_INCOME, "net_income_period_end", researchDate);
    ...
    private void forceThemeLayerIfNeeded(ObjectNode root) {
        ...
        JsonNode layerNode = root.get(FIELD_LAYER);
        if (layerNode != null && layerNode.isInt()) {
            originalLayer = layerNode.asInt();
        }
        boolean changed = originalLayer == null || originalLayer != 3;
        String previousLayerNotes = textOrNull(root, FIELD_LAYER_NOTES);
        if (changed) {
            root.put(FIELD_LAYER, 3);
        }
        String layerNotes = previousLayerNotes;
        if (layerNotes == null || shouldOverwriteThemeNotes(layerNotes)) {
            root.put(FIELD_LAYER_NOTES, "Thematic ETF");
        }
        if (changed) {
            root.put(FIELD_LAYER_NOTES, appendPostprocessorHint(textOrNull(root, FIELD_LAYER_NOTES), match));
            appendWarningNode(root, buildThemeOverrideWarningMessage(match));
        }
        String updatedLayerNotes = textOrNull(root, FIELD_LAYER_NOTES);
    ...
    private void normalizeEpsHistory(ObjectNode valuation, String researchDate) {
        ArrayNode epsHistory = arrayNode(valuation, FIELD_EPS_HISTORY);
        if (epsHistory == null) {
            return;
        }
        ...
        valuation.set(FIELD_EPS_HISTORY, cleaned);
    }
    ...
    private void computePeCurrentIfMissing(ObjectNode valuation, String researchDate) {
        ...
        JsonNode peCurrent = valuation.get(FIELD_PE_CURRENT);
        if (peCurrent != null && !peCurrent.isNull()) {
            return;
        }
        JsonNode priceNode = valuation.get(FIELD_PRICE);
        ...
        valuation.put(FIELD_PE_CURRENT, peValue);
        if (isNullOrBlank(valuation.get(FIELD_PE_CURRENT_ASOF))) {
            valuation.put(FIELD_PE_CURRENT_ASOF, researchDate);
        }
    }
    ...
    private void clearSingleStockHoldingsFields(ObjectNode valuation) {
        setNull(valuation, FIELD_PE_TTM_HOLDINGS);
        setNull(valuation, FIELD_EARNINGS_YIELD_TTM_HOLDINGS);
        setNull(valuation, FIELD_HOLDINGS_COVERAGE_WEIGHT_PCT);
        setNull(valuation, FIELD_HOLDINGS_COVERAGE_COUNT);
        setNull(valuation, FIELD_HOLDINGS_ASOF);
        setNull(valuation, FIELD_HOLDINGS_WEIGHT_METHOD);
    }
    ...
    public KnowledgeBaseLlmAlternativesDraft findAlternatives(String isin, List<String> allowedDomains) {
        ...
        for (JsonNode item : itemsNode) {
            ...
            JsonNode citations = item.get(FIELD_CITATIONS);
