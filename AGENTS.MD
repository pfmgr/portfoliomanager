...
- Backend changes must always be validated with full integration tests. Preferred commands:
  ...
  2. `docker compose build --progress=plain admin_spring` (runs `./gradlew clean bootJar --no-daemon` inside the backend image and prints full compiler/test/coverage output).
  ...
  6. When Docker commands require it, run them with escalated permissions so `docker compose build` can reach `/var/run/docker.sock`; this is necessary in environments where the user-level daemon socket is blocked.
  7. If `./gradlew` fails locally because of sandbox/network restrictions (wildcard IP errors, socket permissions, etc.), rerun `docker compose --progress=plain build admin_spring` as the documented fallback and treat the container build result as the authoritative verification.
